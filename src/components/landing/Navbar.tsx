...
export default function Navbar() {
  ...
  return (
    ...
    >
      ...
      {/* Hamburger container — solo móvil */}
      <div className="md:hidden flex-1 flex justify-end z-20">
        <button
          className="relative flex flex-col justify-center items-center gap-[6px] w-11 h-11 bg-white/10 hover:bg-white/20 border border-white/20 rounded-lg shrink-0 transition-all cursor-pointer pointer-events-auto shadow-sm"
          onClick={(e) => { e.preventDefault(); e.stopPropagation(); setMenuOpen(!menuOpen); }}
          aria-label="Abrir menú"
          aria-expanded={menuOpen}
        >
          <span className={`block w-[24px] h-[2px] bg-pearl rounded-full transition-all ${menuOpen ? "translate-y-[8px] rotate-45" : ""}`}/>
          <span className={`block w-[24px] h-[2px] bg-pearl rounded-full transition-all ${menuOpen ? "opacity-0 scale-x-0" : ""}`}/>
          <span className={`block w-[24px] h-[2px] bg-pearl rounded-full transition-all ${menuOpen ? "-translate-y-[8px] -rotate-45" : ""}`}/>
        </button>
      </div>
